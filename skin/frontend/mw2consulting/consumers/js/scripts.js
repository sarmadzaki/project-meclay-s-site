...
Enterprise.TopCart = {
    ...
    handleMouseOut: function (evt) {
        if ($(this.elementHeader).hasClassName('active')) {
    ...
    handleMouseClick: function (evt) {
        if (!$(this.elementHeader).hasClassName('active') && !$(this.container.id).hasClassName('process'))  {
            this.showCart(4);
        }
        else {
            this.hideCart();
    ...
    showCart: function (timePeriod) {
        /* using side logic for show through active class
        /*this.container.parentNode.style.zIndex=992;
        new Effect.SlideDown(this.container.id, { duration: 0.5,
            beforeStart: function(effect) {$( effect.element.id ).addClassName('process');},
            afterFinish: function(effect) {$( effect.element.id ).removeClassName('process'); }
            });*/
        $(this.elementHeader).addClassName('active');
    ...
    hideCart: function () {
        ...
        if (!$(this.container.id).hasClassName('process') && $(this.elementHeader).hasClassName('active')) {
            /* using side logic for hide through active class
        ...
        $(this.elementHeader).removeClassName('active');
    },
	
	showAccount: function (timePeriod) {
        /* using side logic for show through active class
        /*this.container.parentNode.style.zIndex=992;
        new Effect.SlideDown(this.container.id, { duration: 0.5,
            beforeStart: function(effect) {$( effect.element.id ).addClassName('process');},
