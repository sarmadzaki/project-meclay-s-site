...
Instantsearch.prototype = {
	...
	if (transport && transport.responseText) {
		...
		if(response.products.length > 6)   
    		showNumResults.innerHTML = '6 of '+response.products.length+' RESULTS FOR <span class="global-search__results-search-term" style="text-transform:uppercase">'+this.currentSearch+'</span>';
    	else
    	showNumResults.innerHTML = response.products.length + ' of '+response.products.length+' results for <span class="global-search__results-search-term">'+this.currentSearch+'</span>';
	...
	updateProductDisplay: function(products) {
      ...
      var mainProductWapper =  new Element('div', { 'id': 'mainProductWapper', 'class': 'product-item single', 'style': 'display:none'});
      var mainProduct =  new Element('div', { 'id': 'mainProduct', 'class': 'product-item-container', 'style': 'display:none'});
      mainProductWapper.appendChild(mainProduct);
      productContainer.appendChild(mainProductWapper);
      var display = '';
	    for (var i = 0; i < numThumbs; i++) {
          ...
          mainProductWapper =  new Element('div', { 'id': 'mainProductWapper'+productId, 'class': 'product-item single', 'style':display});
          mainProduct =  new Element('div', { 'id': 'mainProduct'+productId, 'class': 'product-item-container'});
          mainProductWapper.appendChild(mainProduct);
          productContainer.appendChild(mainProductWapper);   
	    }
	...
	loadProductDetail: function(productId,moreProductNum){			
		...
		/* all products will be displaying as main product
		...
		*/
		...
		new Ajax.Updater("mainProductWapper"+productId, url, {method: 'get', onFailure: "",onComplete:this.doneLoadingProduct.bindAsEventListener(this)});
    }
